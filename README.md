...
- `data_process.ipynb`：数据加载与预处理示例（当前为基于 Iris 数据集的随机森林实现与评估，包括混淆矩阵、特征重要性可视化）。
- `Resnet34_classfication.ipynb`：基于 PyTorch 的 ResNet34 二分类训练与验证流程（含数据集划分、数据增强、训练记录与可视化）。
...
### 2. Resnet34_classfication.ipynb
- 主要内容：
...
- Python
- Jupyter Notebook
- data_process.ipynb：numpy / pandas / scikit-learn / matplotlib / seaborn
- Resnet34_classfication.ipynb：torch / torchvision / sklearn / tqdm / matplotlib
